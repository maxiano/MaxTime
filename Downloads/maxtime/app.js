...
const taskDeadline = document.getElementById('task-deadline') || document.getElementById('task-due-date');     
...
function listenToTasksForDate(dateStr) {
    ...
    unsubscribeTasks = onSnapshot(q, (snapshot) => {
        ...
        console.warn("Firestore error for tasks:", error);
        if (Object.keys(latestGroupedTasks).length === 0) {
            const fallbackGrouped = {};
            timeSlots.forEach(slot => fallbackGrouped[slot] = []);
            latestGroupedTasks = fallbackGrouped;
        }
        renderSchedule();
    });
}
...
function listenToNoteForDate(dateStr) {
    ...
    unsubscribeNote = onSnapshot(noteRef, (docSnap) => {
        ...
        console.warn("Firestore error for notes:", error);
        isSavingNote = false;
        noteStatus.textContent = "Offline";
    });
}
...
window.editResolution = async function(id, currentRes) {
    ...
    if (newRes !== null) {
        try {
            await updateDoc(doc(db, "tasks", id), { resolution: newRes.trim() });
        } catch (error) {
            console.error("Errore aggiornamento esito:", error);
        }
    }
};
